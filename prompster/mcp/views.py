...
class MCPToolCallResult(BaseModel):
    ...
    def unwrap(self) -> str:
        text_parts = [b.text for b in self.content if b.type == "text"]

        if self.isError:
            return "Error: " + "\n".join(text_parts) if text_parts else f"Error: {self.model_dump_json()}"

        if text_parts:
            return "\n".join(text_parts)

        if self.structuredContent is not None:
            return self.model_dump_json(include={"structuredContent"})
